...
states_boundingbox = {
    ...
    "Alaska": [51.0228691101074, 71.6048278808594, -180.0, -140.0],
...
def tweets_filter(tweet):
    if ('text' not in tweet) or (tweet['lang'] != 'en'):
        return None # only cares english status tweets
    if not tweet.get('coordinates', None):
        return None # only cares tweet with coordinates
    # return a simplified version of tweet status object
    # {
    #   'text': 'status text',
...
def get_state(coordinate):
    def in_state(coordinate, boundingbox):
        longitude, latitude = coordinate
        a, b, c, d = boundingbox
        return a <= latitude <= b and c <= longitude <= d
    
    for state, boundingbox in states_boundingbox.items():
        if in_state(coordinate, boundingbox):
            return states_abbr.get(state)
    return None


def get_sentiment_score(text, sent_dict):
    return sum(sent_dict.get(word, 0) for word in text.split())
...
def main(sent_file, tweet_file):
    ...
    # 2. pick up tweets with coordinates
    selected_tweets = []
    for tweet in all_tweets:
        t = tweets_filter(tweet)
        if t:
            selected_tweets.append(t)

    # 3. calculate aggragated sentiment score group by state
    states_score = {}
    for tweet in selected_tweets:
        score = get_sentiment_score(tweet['text'], sent_dict)
        state = get_state(tweet['coordinates'])
        if state is None:
            continue
        if state not in states_score:
            states_score[state] = score
        else:
            states_score[state] += score

    # 4. output name of the happiest state
    print sorted(states_score.items(), key=lambda x: x[1], reverse=True)[0][0]
